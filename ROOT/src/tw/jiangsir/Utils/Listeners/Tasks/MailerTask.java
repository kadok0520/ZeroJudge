...
public class MailerTask extends TimerTask {
	...
	public void run() {
		try {
			...
			andrules.add(
					"timestamp>='"
							+ new Utils().parseDatetime(
									new Timestamp(System.currentTimeMillis()
											- CrontabListener.mailer_interval))
					+ "'");
			...
			for (Log log : logs) {
				if (log.getTabid() == Log.TABID.SLOW) {
					...
					if (log.getUri().equals("/favicon.ico")
							|| log.getUri().equals("/robots.txt")
							|| log.getUri().endsWith("IE_pngfix.js")
							|| log.getUri()
									.endsWith("jquery.timeout.interval.idle.js")
							|| log.getUri().endsWith("tiny_mce.js")
							|| log.getUri().endsWith("tinymce.js")
							|| log.getUri()
									.endsWith("jquery.autocomplete.js")) {
						continue;
					}
					errorpage++;
			...
			for (User user : new UserService().getUsersByROLEDEBUGGER()) {
				Mailer mailer = new Mailer(user,
						"Log分析：slow=" + slow + ", errorpage=" + errorpage
								+ ", banned=" + banned + ", gc=" + gc,
						content.toString());
				try {
					mailer.GmailSender();
