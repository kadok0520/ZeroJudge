...
public class UserService {
	...
	public OnlineUser doLogin(HttpSession session, User user) throws DataException {
		...
		GoogleUser googleUser = new SessionScope(session).getGoogleUser();
		AppConfig appConfig = ApplicationScope.getAppConfig();
		if (googleUser == null && user.getAuthhost() != AUTHHOST.localhost && appConfig.getIsGoogleLoginSetup()) {
			throw new DataException("本帳號(" + user.getAccount() + "#" + user.getId() + ") 已經綁定 「" + user.getEmail()
					+ "」，請透過 「" + user.getAuthhost() + "」 認證登入。");
		}
	 ...
	 * 
	 */
	public void doRelogin(HttpSession session) throws DataException {
