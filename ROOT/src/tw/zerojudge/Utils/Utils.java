...
public class Utils {
	 ...
	 * @return
	 */
	public int[] String2IntArray(String intstring)
			throws NumberFormatException {
		if (intstring.matches("^\\[.*\\]$")) {
			intstring = intstring.replaceAll("\\[", "").replaceAll("\\]", "")
					.replaceAll("\"", "").trim();
		}
		if (intstring.equals("")) {
			return new int[] {};
	 ...
	 * @return
	 */
	public double[] String2DoubleArray(String doublestring)
			throws NumberFormatException {
	...
	private boolean isIpInRule(String iprule, IpAddress currentip) {
		...
		if ("*".equals(iprule) || "127.0.0.1".equals(currentip)
				|| currentip.getIp().isLoopbackAddress()) {
			return true;
		}
