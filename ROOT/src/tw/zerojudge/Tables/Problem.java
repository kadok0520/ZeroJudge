...
public class Problem {
	...
	private Language specialjudge_language = new Language("PYTHON");
	...
	public void chekcScores() throws DataException {
		int totalscore = 0;
		for (int score : scores) {
			if (score < 0) {
				throw new DataException("(" + problemid + ") 各題配分必須 >= 0 分");
			}
			totalscore += score;
		}

		if (totalscore != 100) {
			throw new DataException("(" + problemid + ") 分數總和必須為 100 分");
		}
	...
	public void setScores(ArrayList<String> scores) {
		int[] intscores = new int[scores.size()];
		int i = 0;
		for (String score : scores) {
			try {
				intscores[i++] = Integer.parseInt(score);
			} catch (Exception e) {
				intscores[i++] = 0;
			}
		}
	...
	public void setLanguage(String language) {
		...
		this.setLanguage(new Language(language));
	}
	...
	public void setSpecialjudge_language(String specialjudge_language) {
		...
		if ("".equals(specialjudge_language)) {
			this.setSpecialjudge_language(new Language("PYTHON"));
		} else {
			this.setSpecialjudge_language(new Language(specialjudge_language));
		}
	}
	...
	public void checkTimelimits() throws DataException {
		if (this.timelimits.length > 20) {
			throw new DataException("測資數目不可大於 20 個。");
		} else if (this.timelimits.length == 1) {
			double timelimit = timelimits[0];
			...
			for (double timelimit : this.timelimits) {
				if (timelimit <= Problem.MIN_TIMELIMIT) {
					throw new DataException(
							"最低時間限制為 " + Problem.DecimalFormat_ProblemTimeLimit.format(Problem.MIN_TIMELIMIT) + "秒!");
				}
				if (timelimit > Problem.MAX_TIMELIMIT) {
					throw new DataException("單一測資點時間上限為 "
							+ Problem.DecimalFormat_ProblemTimeLimit.format(Problem.MAX_SINGLE_TIMELIMIT) + " 秒!");
				}
