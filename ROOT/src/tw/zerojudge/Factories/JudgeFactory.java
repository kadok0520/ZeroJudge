...
public class JudgeFactory extends SuperFactory<Object> {
	private static PriorityQueue<JudgeObject> JudgeQueue = new PriorityQueue<JudgeObject>(
			1000, new Comparator<JudgeObject>() {
				public int compare(JudgeObject x, JudgeObject y) {
					if (x.getPriority().ordinal() > y.getPriority().ordinal()) {
						...
						if (x.getSolution().getId().intValue() > y
								.getSolution().getId().intValue()) {
							return 1;
						} else {
							return -1;
						}
	...
	public static String getJudgeQueueList() {
		...
		for (JudgeObject judgeObject : JudgeQueue) {
			...
			try {
				sb.append(judgeObject.getProblem().getProblemid() + ":"
						+ judgeObject.getSolution().getId());
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
