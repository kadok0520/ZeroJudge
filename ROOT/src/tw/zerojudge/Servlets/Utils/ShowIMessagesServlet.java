...
public class ShowIMessagesServlet extends HttpServlet implements IAccessFilter {
	...
	public static boolean isAccessible(String session_account)
			throws AccessException {
		User user = new UserService().getUserByAccount(session_account);
		if (user.getIsDEBUGGER() || user.isGeneralManager()) {
			return true;
		}
		return true;
	}
	...
	protected void doGet(HttpServletRequest request,
		...
		synchronized (session) {
			new SessionScope(session).setUnreadIMessages(im.getUnreadIMessage(
					onlineUser, page));
			request.setAttribute("inboxIM",
					im.getInboxIMessages(onlineUser, page));
			request.setAttribute("sentIM",
					im.getSentIMessages(onlineUser, page));
		}
		String action = request.getParameter("action");
		if (action == null || "".equals(action)) {
			request.setAttribute("targetIM",
					im.getInboxIMessages(onlineUser, page));
		} else {
			...
			case unreadIM:
				request.setAttribute("targetIM",
						im.getUnreadIMessage(onlineUser, page));
				break;
			default:
				break;
		...
		request.getRequestDispatcher("ShowIMessages.jsp").forward(request,
				response);
	}
