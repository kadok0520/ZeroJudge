...
function recount() {
	function recount() {
		...
		jQuery("td[id='count']").each(
				function() {
					var index = jQuery("td[id='count']").index(this);
					// jQuery("td[id='count']:eq(" + index + ")").text("#" +
					// (index +
					// 1));
					totalscore += parseInt(jQuery(
							"input[name=scores]:eq(" + index + ")").val());
					jQuery("#totalscore").html(totalscore);
					totaltimelimit += parseInt(jQuery(
							"input[name=timelimits]:eq(" + index + ")").val());
					jQuery("#totaltimelimit").html(totaltimelimit);
				});
